            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/30 text-xs font-bold">
                  {translateDeliveryStatus(trip.status)}
                </span>
                <span className="text-xs text-slate-400">&bull; {t('logistics.assignedDriver')}: {trip.vehicle.vehicleNumber}</span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs text-slate-500 dark:text-slate-400">
                <div>
                  <span className="block text-slate-400">{t('farmer.commodity')}</span>
                  <strong className="text-slate-900 dark:text-white">{translateCategory(trip.commodity)} ({trip.totalKg} kg)</strong>
                </div>
                <div>
                  <span className="block text-slate-400">{t('logistics.reeferClimate')}</span>
                  <strong className="text-emerald-500">{trip.coldChainTemp}°C ({t('logistics.lowSpoilageRisk')})</strong>
                </div>
                <div>
                  <span className="block text-slate-400">{t('farmer.roadDistance')}</span>
                  <strong className="text-slate-900 dark:text-white">{trip.distanceCompletedKm} / {trip.totalDistanceKm} km</strong>
                </div>
                <div>
                  <span className="block text-slate-400">{t('farmer.estimatedEta')}</span>
            ...
            <div className="flex flex-col gap-2 shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  title={t('logistics.reportDispute')}
