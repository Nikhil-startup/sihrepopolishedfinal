          ...
          <div className='flex items-center gap-2.5'>
            ...
            <div>
              <span className='text-xs font-bold uppercase tracking-wider text-emerald-400 block'>
                {t('intelligence.decisionEngine')}
              </span>
              <h1 className='text-xl sm:text-2xl font-black tracking-tight text-white'>
                {commodity} - {t('intelligence.marketIntelligence')}
              </h1>
            </div>
          </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-12 gap-6 items-center'>
          ...
          <div className='md:col-span-6 bg-white/5 backdrop-blur-md rounded-2xl p-5 sm:p-6 border border-white/10 space-y-3'>
            ...
            <div className='flex items-baseline gap-2.5'>
              ...
              <span className='ml-2 text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'>
                {weatherShock.shockMultiplier}x {t('intelligence.shockMultiplier')}
              </span>
            </div>

            <div className='flex flex-wrap items-center gap-4 text-xs pt-1 border-t border-white/10'>
              <div>
                <span className='text-slate-400 block text-[11px]'>{t('intelligence.minSustainablePriceLabel')}</span>
              ...
              <div>
                <span className='text-slate-400 block text-[11px]'>{t('intelligence.sellingRangeLabel')}</span>
          ...
          <div className='md:col-span-6 grid grid-cols-2 gap-3 text-xs'>
            ...
            <div className='bg-white/5 backdrop-blur-sm p-4 rounded-xl border border-white/10 space-y-1'>
              <span className='text-[10px] text-slate-400 uppercase font-semibold block'>{t('intelligence.netProfit')}</span>
              <span className='text-xl font-black text-emerald-400 block'>
                +₹{profitability.netProfit.toLocaleString()}
              </span>
              <span className='text-[11px] text-emerald-300/80'>{t('intelligence.margin', { margin: profitability.profitMarginPercent, health: profitability.marginHealth })}</span>
            </div>

            <div className='bg-white/5 backdrop-blur-sm p-4 rounded-xl border border-white/10 space-y-1'>
              <span className='text-[10px] text-slate-400 uppercase font-semibold block'>{t('intelligence.shortagePercent')}</span>
              <span className='text-xl font-black text-amber-400 block'>
                {weatherShock.marketShortagePercent}% {t('market.deficitAlert') || 'Shortage'}
              </span>
              <span className='text-[11px] text-slate-400'>{t('intelligence.damage', { percent: weatherShock.damageRatePercent, damagedKg: harvestOverview.damagedKg })}</span>
            </div>

            <div className='bg-white/5 backdrop-blur-sm p-4 rounded-xl border border-white/10 space-y-1 col-span-2'>
              <div className='flex items-center justify-between'>
                <span className='text-[10px] text-slate-400 uppercase font-semibold'>{t('intelligence.recommendedAction')}</span>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2'>
          ...
          <div className='bg-white/5 backdrop-blur-md rounded-2xl p-4 sm:p-5 border border-emerald-500/30 flex flex-col justify-between space-y-3'>
            <div>
              <div className='flex items-center justify-between'>
                <span className='text-[10px] uppercase font-bold text-emerald-400 tracking-wider flex items-center gap-1'>
                  <Building2 className='w-3.5 h-3.5' /> {t('intelligence.optimalBuyer')}
                </span>
                <span className='text-xs font-bold px-2 py-0.5 rounded-full bg-emerald-500/30 text-emerald-200'>
                  {t('intelligence.match', { score: topBuyer.compositeScore })}
                </span>
              </div>
            ...
            <div className='flex items-center justify-between border-t border-white/10 pt-3'>
              <div>
                <span className='text-[10px] text-slate-400 block'>{t('intelligence.netRealizationPerKg')}</span>
              ...
              >
                {t('intelligence.acceptOrder')} <ArrowRight className='w-3.5 h-3.5' />
              </Link>
            </div>
          </div>
          ...
          <div className='bg-white/5 backdrop-blur-md rounded-2xl p-4 sm:p-5 border border-white/10 flex flex-col justify-between space-y-3'>
            ...
            <div className='flex items-center justify-between border-t border-white/10 pt-3'>
              ...
              >
                {t('intelligence.dispatchFleet')} <ArrowRight className='w-3.5 h-3.5' />
              </Link>
            </div>
          </div>
